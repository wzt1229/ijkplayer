...
#define BLURAY_PROTO_PREFIX     "bluray2://"
...
#define OFFSET(x) offsetof(IJKBlurayContext, x)
...
static const AVClass ijk_bluray_context_class = {
    .class_name     = "bluray2",
...
{
    IJKBlurayContext *bd = h->priv_data;
...
static int bluray_close(URLContext *h)
{
    ...
    ijk_destroy_bluray_custom_access(&bd->access);
    return 0;
}
...
static int bluray_open(URLContext *h, const char *path, int flags, AVDictionary **options)
{
    ...
    */
    ...
    /* load title list */
    int num_title_idx = bd_get_titles(bd->bd, TITLES_RELEVANT, MIN_PLAYLIST_LENGTH);
